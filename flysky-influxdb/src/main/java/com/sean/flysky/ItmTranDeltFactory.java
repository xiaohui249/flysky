...
public class ItmTranDeltFactory {
    ...
    public static ItmTranDelt create() {
        ItmTranDelt itmTranDelt = new ItmTranDelt();
        itmTranDelt.setMonSid(String.valueOf(worker.nextId()));

        int targetIndex = new Random().nextInt(BASE_NUM) % targetIpAndPorts.size();
        String[] ipAndPort = targetIpAndPorts.get(targetIndex).split(":");
        itmTranDelt.setTargetIp(ipAndPort[0]);
        itmTranDelt.setTargetPort(ipAndPort[1]);

        int sourceIndex = new Random().nextInt(BASE_NUM) % sourceIps.size();
        itmTranDelt.setSourceIp(sourceIps.get(sourceIndex));

        itmTranDelt.setTranTimeStamp(Instant.now());

        int tranNameIndex = new Random().nextInt(BASE_NUM) % tranNames.size();
        itmTranDelt.setTranName(tranNames.get(tranNameIndex));

        int channelIndex = new Random().nextInt(BASE_NUM) % channels.size();
        itmTranDelt.setChannel(channels.get(channelIndex));

        int cost = new Random().nextInt(BASE_NUM_FOR_COST);
        if(cost < 20) {
            itmTranDelt.setTranCost(0);
            itmTranDelt.setRetCode("");
            itmTranDelt.setRetMsg("");
            itmTranDelt.setTranResult("8");
            itmTranDelt.setExcpStat("N");
        } else {
            itmTranDelt.setTranCost(cost);
            int random = new Random().nextInt(BASE_NUM_RET_CODE);
            if(random < 1000) {
                itmTranDelt.setRetCode("0000");
                itmTranDelt.setRetMsg("成功");
                itmTranDelt.setTranResult("0");
                itmTranDelt.setExcpStat("S");
            } else if(random < 2000) {  //1xxx
                itmTranDelt.setRetCode(String.valueOf(random));
                itmTranDelt.setRetMsg("程序异常");
                itmTranDelt.setTranResult("1");
                itmTranDelt.setExcpStat("E");
            } else if(random < 3000) {  //2xxx
                itmTranDelt.setRetCode(String.valueOf(random));
                itmTranDelt.setRetMsg("业务处理失败");
                itmTranDelt.setTranResult("2");
                itmTranDelt.setExcpStat("S");
            }else { //3xxx
                itmTranDelt.setRetCode(String.valueOf(random));
                itmTranDelt.setRetMsg("未知异常");
                itmTranDelt.setTranResult("3");
                itmTranDelt.setExcpStat("S");
            }
        }

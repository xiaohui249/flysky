...
public final class LruCache<K, V> {
    ...
    */
    ...
    static class KeyIterator<K, V> implements Iterator<K> {
        private cache.LruCache<K, V> _cache;
        private CacheItem<K, V> _item;
        private boolean _isHead1;

        KeyIterator(cache.LruCache<K, V> cache) {
            init(cache);
        }

        void init(cache.LruCache<K, V> cache) {
            _cache = cache;
    ...
    static class ValueIterator<K, V> implements Iterator<V> {
        ...
        ValueIterator(cache.LruCache<K, V> cache) {
            init(cache);
        }
    ...
    class EntryIterator implements Iterator<Entry<K, V>>, Entry<K, V> {
        ...
        public void remove() {
            if (_i < _entries.length) {
                ...
                if (entry != null)
                    cache.LruCache.this.remove(entry._key);
            }
        }
    }
}
